...
function IntervalTimer({inhale, exhale, delay, pause, playing, mutedSound}) {
  ...
  const changeStep = () => {
    if(steps.inhale) setSteps(() => ({inhale: false, delay: true, exhale: false, pause: false, duration: delay, color: '#ED7D31', audioTick: audioTick2}))
    ...
    if(steps.exhale) setSteps(() => ({inhale: false, delay: false, exhale: false, pause: true, duration: pause, color: '#00B050', audioTick: audioTick2}))
